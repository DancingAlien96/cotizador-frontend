...
const FILE = path.join(DATA_DIR, "tienda.json");

async function readAll(): Promise<SavedTienda[]> {
  try {
    const raw = await fs.readFile(FILE, "utf8");
    const parsed = JSON.parse(raw) as SavedTienda[];
...
async function writeAll(items: SavedTienda[]): Promise<void> {
